...
% verifier que la coordonne est modifiable (coorX, coorY, Sudoku) 
		% pourrait etre avec coordonnes de 0 a 8 ou de 1 a 9
modifiable(_,_,_).

% verifier si sudoku n'est pas complet (Sudoku)
pasFini(_).
...
% verification du sudoku post modification 	**
numOK(X,Y,S):- BX is floor(X/3), BY is floor(Y/3), 
	getBlock(BX,BY,S,L1), getLine(Y,S,L2), getColumn(X,S,L3),
	verif(L1,_), verif(L2,_), verif(L3,_).
numOK(_,_,_):- write('Valeur incorrecte!!'),nl, fail.

% verification du numero et affichage d'erreur
isValid(X):- valid(X).
isValid(_):- nl, write('Donnee invalide!!!'), nl, nl, fail.

% input de coordonnes par l'utilisateur et affichage d'erreur	**
inCoo(X1,Y1,S):- write('Entrer coordonnee X: '), read(X), isValid(X),nl,
	write('Entrer coordonnee Y: '), read(Y), isValid(Y),nl, modifiable(X,Y,S),
	X1 is (X-1), Y1 is (Y-1), !.
inCoo(_,_):- write('Numero non modifiable'), nl, fail.
...
handle(1):- write('---- Resolution sudoku ----'), nl, sudokuInitial(S),
...
handle1(1,S):- inCoo(X,Y,S), write('Entrer numero: '), read(N), isValid(N),
	changer(N,X,Y,S,S1), numOK(X,Y,S1),
	retract(sudokuGrid(S)), asserta(sudokuGrid(S1)), !.
handle1(1,_):- !, fail.

% Effacer un numero
handle1(2,S):- inCoo(X,Y,S), changer(' ',X,Y,S,S1), 
	retract(sudokuGrid(S)), asserta(sudokuGrid(S1)), !.
handle1(2,_):- !, fail.
	
handle1(4,_).
handle1(_,_):- write('---- Vous avez mal choisi ----'), nl, !.


	%% ---AUTRES2--- %%

prtch(V,X,Y,I):- changer(V,X,Y,I,O), affS(O).
ok(X,Y):-sudokuInitial(S), numOK(X,Y,S).
